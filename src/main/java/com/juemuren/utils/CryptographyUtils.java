...
import com.juemuren.utils.enums.CipherDicEnum;
import com.juemuren.utils.enums.CipherDicTestEnum;
...
public class CryptographyUtils extends BaseUtils {
    ...
    private static final String SEC_PASSWORD =
            "^(?=.*?[0-9])(?=.*?[a-z])(?=.*?[A-Z])(?=.*?[@!#$%^&*()_+\\.\\-\\?<>'\"|=]+).{8,15}$";
    private static String xorKey="Sx###**";
     ...
     * 对加密后的密文做特殊处理
     */
    public static String handleParamAfterEncrypt(final String params, int env) {
        if (StringUtils.isBlank(params)) {
            return null;
        }

        String content = params;   //截取需要做处理的密文
        String overContent = "";  //截取后剩余Content
        if (params.length() > 100) {
            content = params.substring(0, 100);
            overContent = params.substring(100);
        }

        int threshold = content.length() % 2 == 1 ? (content.length() - 1) / 2 + 1 : content.length() / 2;

        //需要异或运算的Content
        String xorData = xor(content.substring(0, threshold));
        //字典替换Content
        String replaceData = env == 1 ? CipherDicTestEnum.replaceDic(content.substring(threshold), 1)
                : CipherDicEnum.replaceDic(content.substring(threshold), 1);
        //位数标志
        String lengthStr = String.format("%04d", xorData.length()) + String.format("%04d", replaceData.length());

        return lengthStr + xorData + replaceData + overContent;
    }

    private static String xor(String xorData) {
        try {
            StringBuilder stringBuilder = new StringBuilder();
            char[] keys = xorKey.toCharArray();
            char[] data = xorData.toCharArray();
            for (int i = 0; i < data.length; i++) {
                stringBuilder.append((char) (data[i] ^ keys[i % keys.length]));
            }
            return Base64.getEncoder().encodeToString(stringBuilder.toString().getBytes());
        } catch (Exception e) {
            logger.error("异或运算异常{}", e);
        }
        return "";
    }
}
